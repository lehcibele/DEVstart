...
class Temperatura {
  ...
  obterLocalizacao() {
    let nomeCidade = this.dados.location.name;
    let nomePais = this.dados.location.country;
    return `${nomeCidade}, ${nomePais}`;
  }

  obterIcone() {
    return this.dados.current.weather_icons;
  }

  obterTemperatura() {
    return `${this.dados.current.temperature}&deg;C`
  }
}
